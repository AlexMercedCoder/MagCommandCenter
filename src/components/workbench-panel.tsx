    ...
    <section className="two-column">
          ...
          <div className="prompt-grid">
            {recipePrompts.map((recipe) => (
              <button
                ...
                key={recipe.name}
                onClick={() => props.onRunRecipe(recipe.command[2])}
                type="button"
              >
                {recipe.name}
              </button>
      ...
      <div className="stack">
        <div className="panel command-panel">
          ...
          <p className="muted">
            Validate and review a portable execution plan before granting its
            gates and checkpoints. Running always requires a final confirmation.
          </p>
          <label htmlFor="graph-path">Graph file</label>
          <div className="field-with-action">
            <input
              id="graph-path"
              value={props.graphPath}
              onChange={(event) => props.setGraphPath(event.target.value)}
              placeholder="plan.agraph.yaml"
            />
            <button
              className="icon-button"
              onClick={props.onChooseGraph}
              type="button"
          ...
          <div className="row-actions">
            <button
              className="icon-action"
              onClick={props.onValidateGraph}
              disabled={props.busy || !props.graphPath.trim()}
              type="button"
            >
              <ShieldCheck size={16} />
              <span>Validate</span>
        ...
        <div className="panel command-panel">
          ...
          <div className="row-actions">
            <button
              className="icon-action"
              onClick={props.onLoadCheckpoints}
              disabled={props.busy}
              type="button"
            >
              <RefreshCcw size={16} />
          ...
          <div className="timeline" aria-label="File checkpoints">
            {props.checkpoints.length ? (
              props.checkpoints.map((checkpoint) => (
                <article
                  className={
                    checkpoint.id === props.selectedCheckpoint
                ...
                >
                  <button
                    className="list-button compact"
                    onClick={() => props.onInspectCheckpoint(checkpoint.id)}
                    type="button"
                  >
                  ...
                  <button
                    ...
                    disabled={props.busy}
                    type="button"
                    title="Restore this file checkpoint"
                  >
                    <Undo2 size={15} />
                    <span>Restore</span>
                  </button>
        ...
        <div className="panel command-panel">
          ...
          <select
            id="peer-target"
            value={props.peerTarget}
            onChange={(event) => props.setPeerTarget(event.target.value)}
          >
          ...
          <textarea
            id="peer-message"
            value={props.peerMessage}
            onChange={(event) => props.setPeerMessage(event.target.value)}
            placeholder="Send bounded coordination context, not hidden instructions or credentials."
          />
          <button
            className="primary-action"
            onClick={props.onSendPeerMessage}
            disabled={
              props.busy || !props.peerTarget || !props.peerMessage.trim()
            }
            type="button"
          >
            <Send size={16} />
        ...
        <div className="panel command-panel">
          ...
          <div className="timeline">
            {props.commandHistory.length ? (
              ...
              <p className="muted">
                Commands run from Command Center will appear here.
              </p>
            )}
          </div>
        </div>
...
export function GraphPlanView(props: {
  value: Record<string, unknown> | null;
}) {
  const nodes = Array.isArray(props.value?.nodes)
    ? props.value.nodes.filter(
  ...
  const isPlan =
    nodes.length > 0 &&
    Array.isArray(props.value?.order) &&
    typeof props.value?.graph_id === "string";
  if (!isPlan) {
    return (
      <JsonPanel
  ...
  const gates = Array.isArray(props.value?.gates)
    ? props.value.gates.map(String)
    : [];
  const table = {
    columns: [
      "step",
    ...
    rows: nodes.map((node, index) => {
      const estimate =
        node.estimate && typeof node.estimate === "object"
          ? (node.estimate as Record<string, unknown>)
          : {};
      return {
        step: index + 1,
        ...
        "estimated cost":
          estimate.cost_usd === undefined
            ? "-"
            : `$${Number(estimate.cost_usd).toFixed(2)}`,
      };
    }),
  };
  return (
    <div className="panel command-panel graph-plan-view">
      ...
      <div className="cockpit-summary" aria-label="Graph plan summary">
        <div>
          ...
          <strong>
            ${Number(props.value?.projected_cost_usd ?? 0).toFixed(2)}
          </strong>
        </div>
        <div>
          <span className="label">Execution bound</span>
          <strong>
            {String(props.value?.worst_case_node_executions ?? nodes.length)}
          </strong>
        </div>
        <div>
          <span className="label">Max parallel</span>
          <strong>{String(props.value?.max_parallel_nodes ?? 1)}</strong>
        </div>
      </div>
      {gates.length > 0 && (
        <p className="recall-reasons">
          <strong>Human gates:</strong> {gates.join(", ")}
        </p>
      )}
