...
export function RunCenterPanel(props: {
  ...
  onCreateSchedule: (path: string, intervalMinutes: number) => Promise<void>;
  onScheduleAction: (
    id: string,
  ...
  const groups = useMemo(
    () => ({
      ...
      active: props.tasks.filter(
        (task) =>
          activeExecutionStates.has(task.state) &&
          !attentionStates.has(task.state),
      ),
      completed: props.tasks.filter(
        (task) =>
          terminalExecutionStates.has(task.state) && task.state !== "failed",
      ),
      all: props.tasks,
    }),
    [props.tasks],
  ...
  return (
    <section className="run-center">
      <div className="run-overview">
        <div>
          <span className="run-metric-icon attention">
            <AlertCircle />
          </span>
        ...
        <div>
          <span className="run-metric-icon active">
            <Clock3 />
          </span>
        ...
        <div>
          <span className="run-metric-icon complete">
            <CheckCircle2 />
          </span>
        ...
        <div>
          ...
          <strong>
            {props.schedules.filter((item) => item.enabled).length}
          </strong>
          <small>Schedules</small>
        </div>
      </div>
      {props.recoveredTaskIds.length > 0 && (
        <div className="inline-notice">
          <RotateCcw />
          <span>
            <strong>Recovered after restart</strong>
            <small>
              {props.recoveredTaskIds.length} unfinished task
              {props.recoveredTaskIds.length === 1 ? "" : "s"} reconnected.
            </small>
          </span>
        </div>
      )}
      <div className="run-layout">
        <section className="run-list-panel">
          <div className="run-tabs" role="tablist">
            {(["attention", "active", "completed", "all"] as Filter[]).map(
              (item) => (
                <button
          ...
          <div className="run-list">
            {groups[filter].length ? (
              groups[filter].map((task) => (
                <button
                  className={
                    selected?.id === task.id ? "run-row active" : "run-row"
                  }
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      {task.kind} · attempt {task.attempt}
                    </small>
                  </span>
              ...
              <div className="empty-state">
                ...
                <p>This project has no {filter} runs.</p>
              </div>
            )}
          </div>
        </section>
      ...
      <section className="panel schedule-manager">
        ...
        <div className="schedule-form">
          <input
            value={schedulePath}
            onChange={(event) => setSchedulePath(event.target.value)}
            placeholder="project graph path (.agraph.yaml)"
          />
          <label>
            Every{" "}
            <input
          ...
          <button
            ...
            disabled={!schedulePath.trim()}
            onClick={() =>
              void props
                .onCreateSchedule(schedulePath.trim(), scheduleInterval)
                .then(() => setSchedulePath(""))
            }
            type="button"
          >
        ...
        <div className="schedule-list">
          {props.schedules.map((item) => (
            <div className="schedule-row" key={item.id}>
              <span
                className={`task-state ${item.approvalPending ? "awaiting_human" : item.enabled ? "running" : "paused"}`}
              />
              <span>
                <strong>{item.path}</strong>
                <small>
                  Every {item.intervalMinutes}m · {item.timezone} · next{" "}
                  {new Date(item.nextRunAt).toLocaleString()}
                  {item.lastStatus ? ` · ${item.lastStatus}` : ""}
                </small>
                {item.lastError && (
                  <small className="error-text">{item.lastError}</small>
                )}
              </span>
              <div className="quiet-actions">
                {item.approvalPending && (
                  <button
                    onClick={() => props.onScheduleAction(item.id, "approve")}
                    type="button"
                  >
                    <Play />
                    Approve run
                ...
                <button
                  onClick={() => props.onScheduleAction(item.id, "run")}
                  type="button"
                >
                  <RotateCcw />
                  Run now
                ...
                <button
                  ...
                  onClick={() => props.onScheduleAction(item.id, "delete")}
                  type="button"
                >
                  <Trash2 />
                  Delete
          ...
          {!props.schedules.length && (
            <p className="empty-copy">
              No schedules. Gate-free graphs can run automatically; graphs with
              human gates wait for approval at each due time.
            </p>
          )}
        </div>
      </section>
    ...
    return (
      <aside className="run-inspector">
        <div className="empty-state">
          ...
          <p>Its state, controls, evidence, and artifacts will appear here.</p>
        </div>
      </aside>
    );
    ...
    <aside className="run-inspector">
      <header>
        ...
        <div className="quiet-actions">
          ...
          {["waiting", "blocked", "awaiting_human"].includes(task.state) && (
            <button
      ...
      {task.files_changed.length > 0 && (
        <section className="artifact-section">
          <h3>Artifacts and files</h3>
          <div>
            {task.files_changed.map((path) => (
              <button
      ...
      {drawer === "activity" ? (
        <ol className="run-events">
          {props.events
            ...
            .reverse()
            .map((event) => (
              <li key={`${event.task_id}-${event.sequence}`}>
                <span className={`task-state ${event.state}`} />
                <div>
                  <strong>{event.type.replace(/_/g, " ")}</strong>
                  <small>{event.created_at}</small>
                  {Object.keys(event.detail).length > 0 && (
                    <p>{summarize(event.detail)}</p>
                  )}
                </div>
              </li>
            ))}
