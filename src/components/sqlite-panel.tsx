      ...
      <div className="panel browser-hero">
        <div>
          ...
          <p>
            ...
            read-only results in the main workspace.
          </p>
        </div>
      ...
      <div className="browser-grid sqlite-browser-grid">
        <div className="panel browser-sidebar">
          ...
          <select
            ...
            onChange={(event) => props.setSelectedDb(event.target.value)}
          >
          ...
          <button
            ...
            disabled={props.busy || !props.selectedDb}
            type="button"
          >
            <Search size={16} />
        ...
        <div className="panel browser-main">
          ...
          <div className="row-actions">
            <button
              ...
              disabled={props.busy || props.page === 0}
              type="button"
            >
              <RefreshCcw size={16} />
              <span>Prev</span>
            ...
            <div>
              ...
              <strong>
                {props.resultRows.rows.length
                  ? `${props.resultRows.rows.length} rows`
                  : "No rows yet"}
              </strong>
            </div>
            <select
              ...
              onChange={(event) =>
                props.setExportFormat(event.target.value as "json" | "csv")
              }
            >
          ...
          <details className="panel inline-details">
            ...
            <pre>
              {props.resultRows.rows.length
                ...
                : "Run a query to prepare JSON/CSV export text."}
            </pre>
          </details>
        </div>
      </div>
        ...
        props.rows.slice(0, 30).map((row, index) => {
          ...
          return table ? (
            <button
              className="list-button compact"
              key={`${table}-${index}`}
              onClick={() => props.setQuery(`select * from ${table}`)}
              type="button"
            >
              <strong>{table}</strong>
...
export function formatExport(table: TableData, format: "json" | "csv") {
  ...
  return [
    ...
    ...table.rows.map((row) =>
      table.columns.map((column) => escape(row[column])).join(","),
    ),
  ].join("\n");
}
