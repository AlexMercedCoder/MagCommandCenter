...
export function emptyProfileDraft(contract?: ProfileContract | null): ProfileDraft {
  ...
  return {
    ...
    objectives: "Complete the requested outcome,Verify important work,Explain the result",
    constraints: "Respect active permissions,Do not claim work was completed without evidence",
...
export function applyTemplate(draft: ProfileDraft, template: string): ProfileDraft {
  const presets: Record<string, Partial<ProfileDraft>> = {
    general: { toolPacks: ["files", "shell", "web", "data", "db", "desktop"], network: "read", memoryMode: "read_write" },
    coder: { toolPacks: ["files", "shell", "web"], network: "read", memoryMode: "read_write", persona: "Focused, pragmatic, and careful with project changes." },
    researcher: { toolPacks: ["files", "web"], network: "read", memoryMode: "read", persona: "Curious, source-conscious, and concise." },
    reviewer: { toolPacks: ["files"], network: "none", memoryMode: "read", allowSubagents: false, persona: "Independent, skeptical, and evidence-driven." },
    custom: { toolPacks: [], network: "none", memoryMode: "off", allowSubagents: false }
  };
  return { ...draft, template, ...(presets[template] ?? {}) };
}

export function buildProfileDocument(draft: ProfileDraft, contract: ProfileContract): OapDocument {
  const toolNames = Array.from(new Set(
    [...draft.explicitTools, ...draft.toolPacks.flatMap((name) => contract.choices.tool_packs.find((pack) => pack.name === name)?.tools ?? [])]
  ));
  ...
  const document: OapDocument = {
    ...
    metadata: {
      ...
      description: draft.description.trim() || `${draft.title || draft.name} agent profile`,
      revision: 1,
      annotations
    },
    spec: {
      role: {
        instructions: draft.instructions.trim() || `Act as ${draft.title || draft.name} and complete work within the declared capabilities.`,
      ...
      runtime: {
        ...
        subagents: draft.allowSubagents
          ? { allow: draft.subagents, max_subagents: draft.maxSubagents, max_parallel: draft.maxParallel, max_depth: draft.maxDepth }
...
export function draftFromDocument(profile: OapDocument, scope = "user"): ProfileDraft {
  ...
  const subagents = typeof runtime.subagents === "object" && runtime.subagents ? runtime.subagents as Record<string, unknown> : {};
  ...
  const refs = (value: unknown) => list(value).map((item) =>
  ...
  return {
    ...
    title: String(annotations["dev.magcommandcenter.title"] ?? profile.metadata.name),
    ...
    scope,
    extends: typeof profile.extends === "string" ? [profile.extends] : profile.extends ?? [],
    ...
    allowSubagents: Number(subagents.max_subagents ?? 0) > 0 || refs(subagents.allow).length > 0,
    ...
    maxStateTokens: Number((context.budget as Record<string, unknown> | undefined)?.max_state_tokens ?? 1200),
    contextFiles: list(context.files).map((item) => typeof item === "object" && item !== null ? String((item as Record<string, unknown>).path ?? "") : String(item)).filter(Boolean).join(","),
