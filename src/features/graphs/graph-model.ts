...
export function graphStages(document: AgenticGraphDocument): {
  stages: GraphStage[];
  cycle: boolean;
} {
  ...
  while (ready.length) {
    ...
    const level = Math.max(
      0,
      ...(document.nodes[id].depends_on ?? []).map(
        (parent) => (levels.get(parent) ?? -1) + 1,
      ),
    );
    levels.set(id, level);
  ...
  return {
    ...
    cycle: seen !== ids.length,
  };
}

export function fallbackNodeTemplate(
  type: GraphNodeType,
  index = 1,
): AgenticGraphNode {
  const base: AgenticGraphNode = {
    type,
    title: `New ${type} ${index}`,
    description: `Configure the ${type} outcome and review it before execution.`,
  };
  if (type === "task")
    return {
      ...base,
  ...
  if (type === "decision")
    return {
      ...
      decision: {
        ...
        branches: [
          { label: "continue", description: "Continue with the default path." },
        ],
      },
    };
  if (type === "gate")
  ...
  const body = {
    entrypoints: ["work"],
    nodes: { work: fallbackNodeTemplate("task") },
  };
  if (type === "loop")
    return { ...base, loop: { mode: "repeat", max_iterations: 1, body } };
  if (type === "map")
    return {
      ...base,
      map: { over: "[]", as: "item", max_items: 10, max_parallel: 1, body },
    };
...
export function localDraftFromGoal(goal: string): AgenticGraphDocument {
  ...
  const nodes: AgenticGraphDocument["nodes"] = {
    inspect: {
      ...fallbackNodeTemplate("task"),
      title: "Inspect context",
      description: `Understand the project and constraints for: ${objective}`,
      labels: ["discovery"],
    },
    plan: {
      ...fallbackNodeTemplate("task"),
      title: "Plan the change",
      description: "Turn the findings into a reviewable implementation plan.",
      depends_on: ["inspect"],
      labels: ["planning"],
    },
    implement: {
      ...fallbackNodeTemplate("task"),
      title: "Implement",
      description: "Apply the approved changes with bounded authority.",
      depends_on: ["plan"],
      requirements: {
        tools: ["file_read", "file_edit", "shell_exec"],
        permissions: ["fs:read:**", "fs:write:project"],
    ...
    verify: {
      ...fallbackNodeTemplate("task"),
      title: "Verify",
      description: "Run focused tests and collect evidence for the result.",
      depends_on: ["implement"],
      labels: ["quality"],
    },
    review: {
      ...fallbackNodeTemplate("gate"),
      title: "Review evidence",
      description:
        "Review the change and its validation evidence before completion.",
      depends_on: ["verify"],
      labels: ["approval"],
    },
  };
  return {
    ags_version: "1.0",
    kind: "AgenticGraph",
    id: "local/review-draft",
    title: objective.length > 56 ? `${objective.slice(0, 53)}…` : objective,
    objective,
    entrypoints: ["inspect"],
    nodes,
  };
...
export function addNode(
  ...
  template?: AgenticGraphNode,
): AgenticGraphDocument {
  const id = nextNodeId(document, type);
  const node = structuredClone(
    template ??
      fallbackNodeTemplate(type, Object.keys(document.nodes).length + 1),
  );
  node.title = node.title.replace(
    /\d+$/,
    String(Object.keys(document.nodes).length + 1),
  );
  return {
    ...document,
    nodes: { ...document.nodes, [id]: node },
    entrypoints: document.entrypoints.length ? document.entrypoints : [id],
  };
}
...
export function updateNode(
  document: AgenticGraphDocument,
  id: string,
  patch: Partial<AgenticGraphNode>,
  replace = false,
): AgenticGraphDocument {
  return {
    ...document,
    nodes: {
      ...document.nodes,
      [id]: replace
        ? (patch as AgenticGraphNode)
        : { ...document.nodes[id], ...patch },
    },
  };
}
...
export function removeNode(
  ...
  const nodes = Object.fromEntries(
    Object.entries(document.nodes)
      ...
      .map(([nodeId, node]) => [
        ...
        {
          ...node,
          depends_on: (node.depends_on ?? []).filter((parent) => parent !== id),
        },
      ]),
  );
  ...
  return {
    ...
    nodes,
    edges,
    entrypoints: document.entrypoints.filter((entry) => entry !== id),
  };
}

export function duplicateNodeLocal(
  document: AgenticGraphDocument,
  id: string,
): { document: AgenticGraphDocument; id: string } {
  let suffix = 2;
  let next = `${id}_copy`;
  ...
  return {
    document: { ...document, nodes: { ...document.nodes, [next]: copy } },
    id: next,
  };
}
    ...
    for (const parent of node.depends_on ?? []) {
      if (!ids.has(parent))
        result.push({
          ...
          severity: "error",
          nodeId: id,
          message: `${id} depends on missing node ${parent}.`,
        });
    ...
    for (const output of Object.keys(node.outputs ?? {})) {
      ...
      if (!serialized.includes(reference))
        result.push({
          ...
          severity: "warning",
          nodeId: id,
          message: `${id}.${output} is declared but not consumed or exported.`,
        });
    }
  }
  ...
  for (const id of ids)
    if (!reachable.has(id))
      result.push({
        ...
        severity: "warning",
        nodeId: id,
        message: `${id} is unreachable from the graph entrypoints.`,
      });
  return result;
}

export function outgoingDependents(
  document: AgenticGraphDocument,
  id: string,
): string[] {
  return Object.entries(document.nodes)
    .filter(([, node]) => (node.depends_on ?? []).includes(id))
    .map(([child]) => child);
}
  ...
  return new Set(
    Object.entries(document.nodes)
      .filter(([, node]) => {
        ...
        if (profile && (node["x-magagent-profile"] ?? "") !== profile)
          return false;
...
export function jsonSource(document: AgenticGraphDocument): string {
  return JSON.stringify(document, null, 2);
}

export function sourceDiff(
  before: AgenticGraphDocument | null,
  after: AgenticGraphDocument,
): string[] {
