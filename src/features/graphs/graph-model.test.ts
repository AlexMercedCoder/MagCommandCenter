...
const graph: AgenticGraphDocument = {
  ags_version: "1.0",
  kind: "AgenticGraph",
  id: "test/board",
  title: "Board",
  ...
  nodes: {
    ...
    b: { title: "B", description: "B", depends_on: ["a"] },
    c: { title: "C", description: "C", depends_on: ["a"] },
  },
};

describe("graph board model", () => {
  it("groups parallel cards into computed execution stages", () => {
    expect(graphStages(graph)).toEqual({
      stages: [
        { level: 0, nodeIds: ["a"] },
        { level: 1, nodeIds: ["b", "c"] },
      ],
  ...
  it("creates a valid local draft for browser previews", () => {
    ...
    expect(
      graphDiagnostics(draft).filter((item) => item.severity === "error"),
    ).toEqual([]);
  });
  ...
  it("duplicates, filters, diffs, and diagnoses graph drafts", () => {
    ...
    const labeled = updateNode(duplicated.document, "b", {
      labels: ["review"],
      "x-magagent-profile": "docs",
    });
    expect(filterNodeIds(labeled, "", "", "docs", "review")).toEqual(
      new Set(["b"]),
    );
    expect(
      sourceDiff(graph, labeled).some((line) => line.includes("review")),
    ).toBe(true);
    const broken = updateNode(labeled, "b", { depends_on: ["missing"] });
    expect(graphDiagnostics(broken).map((item) => item.code)).toContain(
      "unknown_dependency",
    );
  });
  ...
  it("keeps 500-node graph analysis within the interactive budget", () => {
    ...
    for (let index = 0; index < 500; index += 1)
      nodes[`node_${index}`] = {
        title: `Node ${index}`,
        description: "Work",
        ...(index ? { depends_on: [`node_${index - 1}`] } : {}),
      };
