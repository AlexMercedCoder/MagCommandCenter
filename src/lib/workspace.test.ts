...
describe("workspace domain helpers", () => {
  ...
  it("adds bounded pagination without overriding an explicit limit", () => {
    expect(withPagination("select * from tasks;", 2)).toBe(
      "select * from tasks limit 100 offset 200",
    );
    expect(withPagination("select * from tasks limit 5", 3)).toBe(
      "select * from tasks limit 5",
    );
  });

  it("summarizes cached orchestrated goals with runnable commands", () => {
    const summary = summarizeOrchestratedGoal({
      plan: { id: "plan_1" },
      orchestration: { cache_key: "abc", steps: [{}, {}] },
    });
