import type { ConfigField, MemoryNode, RunArtifact, RunCockpit, RunPermission, RunToolEvent, SqliteDatabase, TableData } from "./types";
...
export function deriveRunCockpit(
  ...
  const tools = mergeTools([...toolsFromEvents(events), ...toolsFromStream(streamLines)]);
  const permissions = mergePermissions([...permissionsFromEvents(events), ...permissionsFromStream(streamLines)]);
  const artifacts = mergeArtifacts([
    ...artifactsFromEvents(events),
    ...artifactsFromResponse(response),
    ...artifactsFromStream(streamLines)
  ]);
  const failedToolCount = tools.filter((tool) => tool.status === "failed" || tool.status === "blocked").length;
  const explicitOk = typeof response?.ok === "boolean" ? response.ok : null;
  const completed = Boolean(response) || events.some((event) => String(event.type ?? "").includes("completed"));
  ...
  const totalDurationMs = durationFromEvents(events) ?? durationFromStream(streamLines);
  const headline = buildRunHeadline({ completed, explicitOk, failedToolCount, toolCount, permissions, artifacts });
...
function toolsFromEvents(events: Array<Record<string, unknown>>): RunToolEvent[] {
  return events.flatMap((event) => {
    const type = String(event.type ?? event.event ?? "");
    const name = String(event.tool ?? event.name ?? event.tool_name ?? "");
    if (!name && !type.includes("tool")) return [];
    const detail = pretty(event.command ?? event.path ?? event.file ?? event.detail ?? event.error ?? event.content ?? "");
    return [
      {
        name: name || type || "tool",
        status: statusFromEvent(event),
        detail,
        durationMs: numberFrom(event.duration_ms ?? event.elapsed_ms ?? event.ms),
...
function toolsFromStream(lines: string[]): RunToolEvent[] {
  return lines.flatMap<RunToolEvent>((line): RunToolEvent[] => {
    const toolStart = line.match(/(?:🔧|tool)\s+([a-zA-Z0-9_-]+)(?:\s+\[[^\]]+\])?\s*(.*)$/);
    if (toolStart) {
      const detail = toolStart[2]?.trim() ?? "";
      return [{ name: toolStart[1], status: "running", detail, path: pathFromText(detail) } satisfies RunToolEvent];
    }
    const finished = line.match(/->\s+([a-zA-Z0-9_-]+)\s+finished\s+in\s+([0-9.]+)\s*(ms|s)\s*(?:\(([^)]*)\))?/);
    if (finished) {
      const duration = Number.parseFloat(finished[2]) * (finished[3] === "s" ? 1000 : 1);
...
function permissionsFromEvents(events: Array<Record<string, unknown>>): RunPermission[] {
  return events.flatMap((event) => {
    const type = String(event.type ?? event.event ?? "").toLowerCase();
    const command = stringFrom(event.command ?? event.run ?? event.shell ?? event.detail);
    if (!type.includes("permission") && !command) return [];
    if (!type.includes("permission") && !String(event.status ?? "").toLowerCase().includes("permission")) return [];
    return [{ command, status: permissionStatus(event), detail: pretty(event.reason ?? event.detail ?? event.status ?? "") }];
  });
}
...
function permissionsFromStream(lines: string[]): RunPermission[] {
  return lines.flatMap((line) => {
    ...
    const command = line.match(/Run:\s*`([^`]+)`/)?.[1] ?? line.match(/run_shell[^\]]*\]\s*(.*)$/)?.[1] ?? "";
    return [{ command, status: /denied|blocked|failed/i.test(line) ? "blocked" : "requested", detail: line }];
  });
}

function artifactsFromEvents(events: Array<Record<string, unknown>>): RunArtifact[] {
  return events.flatMap((event) => {
    const path = stringFrom(event.path ?? event.file ?? event.file_path ?? event.output_path);
    const type = String(event.type ?? event.event ?? event.tool ?? "");
    if (!path || !looksLikeArtifactPath(path)) return [];
    return [{ path, kind: artifactKind(path, type), detail: pretty(event.bytes ?? event.detail ?? event.status ?? type) }];
  });
}

function artifactsFromResponse(response: Record<string, unknown> | null): RunArtifact[] {
  if (!response) return [];
  const candidates = [response.files, response.files_touched, response.artifacts, response.outputs, response.paths];
  return candidates.flatMap((candidate) => artifactList(candidate));
}

function artifactsFromStream(lines: string[]): RunArtifact[] {
  return lines.flatMap((line) => {
    const match = line.match(/(?:write_file|create_docx|create_pptx|create_svg|create_diagram|create_image|generate_image)[^\n]*\s(\/[^\s]+|[\w.-]+\/[^\s]+|[\w.-]+\.[a-zA-Z0-9]+)/);
    if (!match) return [];
    return [{ path: match[1], kind: artifactKind(match[1], line), detail: line }];
  });
}
...
function artifactList(candidate: unknown): RunArtifact[] {
  ...
  return candidate.flatMap((item) => {
    if (typeof item === "string" && looksLikeArtifactPath(item)) return [{ path: item, kind: artifactKind(item), detail: "" }];
    if (typeof item !== "object" || item === null) return [];
    const record = item as Record<string, unknown>;
    const path = stringFrom(record.path ?? record.file ?? record.file_path ?? record.name);
    if (!path || !looksLikeArtifactPath(path)) return [];
    return [{ path, kind: artifactKind(path, stringFrom(record.kind ?? record.type)), detail: pretty(record.detail ?? record.status ?? "") }];
  });
}
...
function mergeTools(tools: RunToolEvent[]) {
  ...
  for (const tool of tools) {
    const relatedKey = Array.from(byKey.keys()).find((key) => {
      ...
      return existing?.name === tool.name && (!existing.path || !tool.path || existing.path === tool.path);
    });
    const key = relatedKey ?? `${tool.name}:${tool.path ?? tool.detail}`;
    const existing = byKey.get(key);
    if (!existing || statusRank(tool.status) >= statusRank(existing.status) || (tool.durationMs ?? 0) > (existing.durationMs ?? 0)) {
      byKey.set(key, { ...existing, ...tool, detail: tool.detail || existing?.detail || "" });
    }
  }
...
function mergePermissions(permissions: RunPermission[]) {
  ...
  for (const permission of permissions) byKey.set(permission.command || permission.detail, permission);
...
function statusFromEvent(event: Record<string, unknown>): RunToolEvent["status"] {
  const text = `${event.status ?? ""} ${event.ok ?? ""} ${event.error ?? ""}`.toLowerCase();
  if (text.includes("permission") || text.includes("blocked")) return "blocked";
  if (text.includes("fail") || text.includes("error") || text.includes("false")) return "failed";
  if (text.includes("ok") || text.includes("true") || text.includes("success")) return "ok";
  if (String(event.type ?? "").toLowerCase().includes("start")) return "running";
  return "unknown";
}

function permissionStatus(event: Record<string, unknown>): RunPermission["status"] {
  const text = `${event.status ?? ""} ${event.type ?? ""} ${event.ok ?? ""}`.toLowerCase();
...
function countModelRounds(events: Array<Record<string, unknown>>, lines: string[]) {
  const eventCount = events.filter((event) => String(event.type ?? event.event ?? "").toLowerCase().includes("model")).length;
  const streamCount = lines.filter((line) => /time model round/i.test(line)).length;
  return Math.max(eventCount, streamCount);
}

function countToolMentions(events: Array<Record<string, unknown>>, lines: string[]) {
  const eventCount = events.filter((event) => String(event.type ?? event.event ?? event.tool ?? "").toLowerCase().includes("tool")).length;
  const streamCount = lines.filter((line) => /(?:🔧|->\s+[a-zA-Z0-9_-]+\s+finished)/.test(line)).length;
  return Math.max(eventCount, streamCount);
}

function durationFromEvents(events: Array<Record<string, unknown>>) {
  const durations = events.map((event) => numberFrom(event.duration_ms ?? event.elapsed_ms ?? event.ms)).filter((item): item is number => typeof item === "number");
  return durations.length ? durations.reduce((total, item) => total + item, 0) : undefined;
}

function durationFromStream(lines: string[]) {
  const durations = lines.flatMap((line) => {
    const match = line.match(/responded in ([0-9.]+)s|finished in ([0-9.]+)\s*(ms|s)/i);
    if (!match) return [];
...
function buildRunHeadline(input: {
  ...
  if (!input.completed) return input.toolCount ? "Running with live tool activity" : "Waiting for MagAgent activity";
  if (input.explicitOk === false || input.failedToolCount) return "Completed with issues to review";
  if (input.permissions.some((permission) => permission.status === "blocked" || permission.status === "denied")) return "Completed after permission friction";
  if (input.artifacts.length) return `Completed with ${input.artifacts.length} artifact${input.artifacts.length === 1 ? "" : "s"}`;
  return "Completed";
}
