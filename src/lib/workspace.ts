...
export function summarizeOrchestratedGoal(
  data: Record<string, unknown> | null,
) {
  if (!data) return "";
  const plan = data.plan as Record<string, unknown> | undefined;
  const orchestration = data.orchestration as
    Record<string, unknown> | undefined;
  const planId = String(plan?.id ?? "");
  const cacheKey = String(orchestration?.cache_key ?? "");
  const steps = Array.isArray(orchestration?.steps)
  ...
  const sessions = value
    .map((item): ChatSession | null => {
      if (typeof item === "string")
        return { id: item, name: item, createdAt: now, updatedAt: now };
